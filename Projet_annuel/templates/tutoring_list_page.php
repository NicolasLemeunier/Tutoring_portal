...
//var_dump($allTutoring);
	foreach ($data as $key) {
		...
		if(isset($_SESSION['user']) && $_SESSION['user']->getStatus() === "Tutor"){
			if($status == "started"){
				echo "<tr><td>Tutorat en {$key['category']}</td><td>{$status}</td>
				<td><button><a href={$this->router->getChatURL($key['id'])}>Rejoindre le chat</a></button><button><a href={$this->router->getEndTutoringURL($key['id'])}>Terminer le tutorat</a></button><button><a href={$this->router->getTutoringInformationPageURL($key['id'])}>Infos</a></button><button><a href={$this->router->getTutoringModificationURL($key['id'])}>Modifer</a></button><button><a href={$this->router->getTutoringDeletionURL($key['id'])}>Supprimer</a></button></td></tr>";
			}else if($status == "not_started" || $status == "ended"){
				echo "<tr><td>Tutorat en {$key['category']}</td><td>{$status}</td>
				<td><button><a href={$this->router->getStartTutoringURL($key['id'])}>Démarrer le tutorat</a></button><button><a href={$this->router->getTutoringInformationPageURL($key['id'])}>Infos</a></button><button><a href={$this->router->getTutoringModificationURL($key['id'])}>Modifer</a></button><button><a href={$this->router->getTutoringDeletionURL($key['id'])}>Supprimer</a></button></td></tr>";
			}
		}else if(isset($_SESSION['user']) && $_SESSION['user']->getStatus() === "Student"){
			if($status == "started"){
				echo "<tr><td>Tutorat en {$key['category']}</td><td>{$key['tutor']}</td><td>{$status}</td><td><button><a href={$this->router->getChatURL($key['id_tutoring'])}>Rejoindre le chat</a></button><button><a href={$this->router->getTutoringInformationPageURL($key['id_tutoring'])}>Infos</a></button><button><a href={$this->router->getLeaveTutoringURL($key['id_tutoring'])}>Quitter</a></button></td></tr>";
			}else if ($status == "not_started" || $status == "ended"){
